...
public class Shark : MonoBehaviour {
	...
	public bool seekEnabled,pursueEnabled, arriveEnabled, pathFollowEnabled, offsetPursuitEnabled; //You can toggle which steering behaviour you want on in the inspector.
	...
	void OnTriggerEnter(Collider other) //If the sharks collider collides with the MainCamera
	{

		if(other.gameObject.tag == "MainCamera")
		{
			StartCoroutine(LoadAfterDelay(5f)); //Load the level after 5 seconds.
			//I am doing this as the shark sometimes gets stuck near the camera so I am resetting the level after 5 seconds.
			
			if(other.gameObject.tag == "Leader")//When the shark arrives at the fish the fish will die.
				//You must set arrive enabled in the inspector. //Once the fish is dead the shark will stay on the sand.
				//Flee must also be disabled on the Leader script for this to work.
			{
				Destroy(gameObject);
			}
		}
			...
			IEnumerator LoadAfterDelay(float waitTime){
				yield return new WaitForSeconds(waitTime); // wait 5 seconds
